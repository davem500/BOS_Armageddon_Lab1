...
locals {
  ...
  larrryharrisaws_zone_name = var.domain_name

  # Explanation: Use either Terraform-managed zone or a pre-existing zone ID (students choose their destiny).
  larrryharrisaws_zone_id = var.manage_route53_in_terraform ? aws_route53_zone.larrryharrisaws_zone01[0].zone_id : var.route53_hosted_zone_id

  # Explanation: This is the app address that will growl at the galaxy (app.bos-growl.com).
  larrryharrisaws_app_fqdn = "${var.app_subdomain}.${var.domain_name}"
}
...
resource "aws_route53_zone" "larrryharrisaws_zone01" {
  count = var.manage_route53_in_terraform ? 1 : 0

  name = local.larrryharrisaws_zone_name
...
resource "aws_route53_record" "larrryharrisaws_acm_validation_records01" {
  ...
  zone_id = local.larrryharrisaws_zone_id
...
resource "aws_acm_certificate_validation" "larrryharrisaws_acm_validation01_dns_bonus" {
  count = var.certificate_validation_method == "DNS" ? 1 : 0

  certificate_arn = aws_acm_certificate.larrryharrisaws_cert.arn

  validation_record_fqdns = [
    for r in aws_route53_record.larrryharrisaws_acm_validation_records01 : r.fqdn
  ]
}